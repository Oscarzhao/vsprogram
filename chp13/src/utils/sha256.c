...
{
    r_uint32 temp1, temp2, W[64];
    r_uint32 A, B, C, D, E, F, G, H;
...
void sha256_update(sha256_context * ctx, const void *input, size_t length)
{
    ...
    if (left && length >= fill) {
        ...
        length -= fill;
        input = (char *)input + fill;
        left = 0;
    }

    while (length >= 64) {
        sha256_process(ctx, (unsigned char *)input);
        length -= 64;
        input = (char *)input + 64;
    }
...
void sha256_get(sha256_context *ctx, unsigned char digest[32])
{
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (i < 2) {
                sha256_update(&ctx, (unsigned char *) msg[i],
                    strlen(msg[i]));
            } else {
                memset(buf, 'a', 1000);

                for (j = 0; j < 1000; j++) {
                    sha256_update(&ctx, (unsigned char *) buf, 1000);
                }
            }
